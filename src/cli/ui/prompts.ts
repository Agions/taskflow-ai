 ...
 * 输入提示
 */
export async function input(message: string, defaultValue?: string, validate?: (value: string) => boolean | string): Promise<string> {
  const { result } = await inquirer.prompt([{
    type: 'input',
    name: 'result',
    message: theme.info(message),
 ...
 * 密码输入
 */
export async function password(message: string, validate?: (value: string) => boolean | string): Promise<string> {
  const { result } = await inquirer.prompt([{
    type: 'password',
    name: 'result',
    message: theme.info(message),
 ...
 * 选择列表
 */
export async function select<T extends string>(message: string, choices: { name: string; value: T; disabled?: boolean }[], defaultValue?: T): Promise<T> {
  const { result } = await inquirer.prompt([{
    type: 'list',
    name: 'result',
    message: theme.info(message),
 ...
 * 多选
 */
export async function multiSelect<T extends string>(message: string, choices: { name: string; value: T; checked?: boolean }[]): Promise<T[]> {
  const { result } = await inquirer.prompt([{
    type: 'checkbox',
    name: 'result',
    message: theme.info(message),
 ...
 * 数字输入
 */
export async function number(message: string, defaultValue?: number, min?: number, max?: number): Promise<number> {
  const { result } = await inquirer.prompt([{
    type: 'number',
    name: 'result',
    message: theme.info(message),
