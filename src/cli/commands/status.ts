...
async function loadTaskFile(
  outputDir: string
): Promise<{ filePath: string; data: TaskFile } | null> {
 ...
 */
async function runStatusNext(options: { count?: string; priority?: string; output?: string }) {
  ...
  try {
    ...
    let candidates = tasks.filter(
      (t: TaskRecord) => t.status !== 'done' && t.status !== 'cancelled'
    );
    ...
    recommended.forEach((task: TaskRecord, index: number) => {
      ...
      console.log(
        chalk.white(`  ${index + 1}. `) + chalk.bold((task.title || '无标题').substring(0, 50))
      );
      console.log(
        chalk.gray('     ID: ') +
...
async function runStatusProgress(options: {
  ...
  try {
    ...
    console.log(
      chalk.white('  整体进度 ') +
        chalk.green(`${progressPercent}%`) +
        ` ${bar} ${chalk.gray(`${doneCount}/${total} 任务`)}`
    );
    console.log();
    ...
    if (options.detailed) {
      ...
      console.log(chalk.gray('    总预计工时: ') + chalk.blue(`${totalHours}h`));
      console.log(chalk.gray('    已完成工时: ') + chalk.green(`${completedHours}h`));
      console.log(chalk.gray('    剩余工时:   ') + chalk.yellow(`${totalHours - completedHours}h`));
      const hoursBar =
        chalk.green('█'.repeat(Math.round((hoursPercent / 100) * barLength))) +
        chalk.gray('░'.repeat(barLength - Math.round((hoursPercent / 100) * barLength)));
      console.log(
        chalk.gray('    工时进度    ') + chalk.green(`${hoursPercent}%`) + ` ${hoursBar}`
      );
    }
...
async function runStatusUpdate(
  ...
  if (!validStatuses.includes(normalizedStatus)) {
    spinner.fail(chalk.red(`无效状态 "${newStatus}"，有效值: ${validStatuses.join(' | ')}`));
    return;
  }
  ...
  try {
    ...
    console.log(chalk.cyan('\n✅ 任务状态更新成功\n'));
    console.log(chalk.gray('  任务ID:   ') + chalk.cyan(task.id?.substring(0, 8)));
    console.log(
...
async function runStatusCurrent(options: { output?: string }) {
  ...
  try {
    ...
    // 查找进行中的任务
    const inProgress = tasks.filter((t: TaskRecord) => t.status === 'in_progress');
    ...
    if (inProgress.length === 0 && highPriorityTodo.length === 0) {
      ...
      console.log(
        chalk.gray('  - 使用 "taskflow status list --filter status=todo" 查看待处理任务')
      );
      return;
    }
    ...
    if (inProgress.length > 0) {
      ...
      inProgress.forEach((task: TaskRecord) => {
        console.log(chalk.cyan('  ▶ ') + chalk.bold((task.title || '无标题').substring(0, 50)));
        console.log(
          chalk.gray('    ID: ') +
    ...
    if (highPriorityTodo.length > 0) {
      ...
      highPriorityTodo.slice(0, 3).forEach((task: TaskRecord) => {
        console.log(chalk.yellow('  ○ ') + chalk.white((task.title || '无标题').substring(0, 45)));
        console.log(
          chalk.gray('    ID: ') +
            chalk.cyan(task.id?.substring(0, 8)) +
            chalk.gray('  工时: ') +
