...
export class MemoryCache<K extends string, V> {
  ...
  constructor(options: MemoryCacheOptions = {}) {
    ...
    logger.info(`MemoryCache 初始化: maxSize=${this.maxSize}, maxMemory=${this.maxMemory} bytes, ttl=${this.defaultTtl}s`);
  }
  ...
  set(key: K, value: V, ttl?: number): void {
    ...
    const entry: CacheEntry<V> = {
      key,
      value,
      expiresAt: ttl ? Date.now() + ttl * 1000 : (this.defaultTtl ? Date.now() + this.defaultTtl * 1000 : null),
      accessCount: 1,
