...
export const httpTools: ToolDefinition[] = [
  {
    ...
    handler: async input => {
      ...
      if (!urlValidation.valid) {
        throw new Error(`URL验证失败: ${urlValidation.reason}`);
      }
  ...
  {
    name: 'http_download',
    description: '下载文件到本地（仅允许 downloads/output/temp 目录）',
    inputSchema: {
      type: 'object',
      properties: {
        url: { type: 'string', description: '文件 URL' },
        path: { type: 'string', description: '保存路径（必须在允许目录内）' },
        timeout: {
          type: 'number',
          description: '超时时间 (毫秒)',
        },
    ...
    handler: async input => {
      ...
      const { validateUrl, validateDownloadPath } = await import('../security/validator');

      // 验证URL安全性（防SSRF）
      const urlValidation = validateUrl(input.url as string);
      if (!urlValidation.valid) {
        throw new Error(`URL验证失败: ${urlValidation.reason}`);
      }

      // 验证下载路径安全性
      const pathValidation = validateDownloadPath(input.path as string);
      if (!pathValidation.valid) {
        throw new Error(`路径验证失败: ${pathValidation.reason}`);
      }
